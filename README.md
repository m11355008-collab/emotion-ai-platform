...
只要設定了 `HF_TOKEN` 和 `HF_BACKUP_DATASET_REPO`，每位參與者完成全部流程時，
系統會自動把他的三份 CSV（對話紀錄、歷程紀錄、作答紀錄）連同目前的分派
狀態檔，一起備份到你指定的私人 Hugging Face Dataset repo，不需要你手動操作。
沒有設定這兩個變數的話，系統會直接跳過備份，不影響其他任何功能——但這代表
你需要自己記得定期從 Space 手動下載資料備份。
